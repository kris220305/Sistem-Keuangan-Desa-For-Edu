...
export default function SPPDefinitif() {
  ...
  const handleSimpan = () => {
    ...
    if (mode === "add") {
      const newItem: SPPItem = { id: crypto.randomUUID(), jenis: "definitif", tanggalSPP: form.tanggalSPP, nomorSPP: form.nomorSPP || generateNoSPP(), uraian: form.uraian, jumlah: form.jumlah, isFinal: false, pembayaran: form.pembayaran, rincian: [], buktiTransaksi: [] };
      save([...items, newItem]);
      setSelected(newItem);
      toast.success("SPP Definitif berhasil disimpan");
          ...
          {activeTab === "potongan" && selected && selectedBukti && (
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="p-4 space-y-2 bg-muted/10">
                ...
                <div className="flex items-center gap-2"><Label className="text-[11px] w-28 shrink-0">Billing Pajak</Label>
                  <Input className="h-7 text-[11px]" readOnly /></div>
              </div>
              <ActionBar

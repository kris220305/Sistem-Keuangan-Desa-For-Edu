...
export default function PendapatanDesa() {
  ...
  const handleRekeningChange = (kode: string) => {
    const rek = rekeningPendapatan.find(r => r.kode === kode);
    setForm({ ...form, kodeRekening: kode, namaRekening: rek?.uraian || "" });
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-4 space-y-3 overflow-auto">
        ...
        <div className="content-card p-4">
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            <div className="space-y-3">
              <div>
                ...
                {mode !== "view" ? (
                  <Select value={form.kodeRekening} onValueChange={handleRekeningChange}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Pilih Rekening Pendapatan" /></SelectTrigger>
                    <SelectContent>{rekeningPendapatan.map(r => <SelectItem key={r.kode} value={r.kode}>{r.kode} — {r.uraian}</SelectItem>)}</SelectContent>
                  </Select>
                ) : (
          ...
          {/* Rincian - only in tambah/ubah mode */}
          {mode !== "view" && (
            <div className="mt-4 pt-3 border-t space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <Input value="01" readOnly className="h-8 text-xs bg-muted" />
                </div>
          ...
          {/* Selected item rincian in view mode */}
